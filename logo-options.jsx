/* AWS Community Day · Accra wordmark — exact Bay Area lockup pattern.
   Bay Area reference: aws smile (orange) → COMMUNITY DAY (white) → CITY (gold) flanked by short rules.
   We honor that hierarchy and show two takes: with and without a Ghana black-star backdrop. */

const TOK = {
  midnight: "#1F2A3D", // dark navy like the Bay Area card
  midnight2: "#192234",
  cream: "#F4ECD8",
  white: "#FFFFFF",
  gold: "#D4A24C",
  awsOrange: "#FF9900",
  ink: "#1F1B16",
  ruleLight: "rgba(255,255,255,0.55)",
};

/* Official AWS Smile — orange smile, white wordmark when on dark, ink on light. */
const AwsSmile = ({ wordmark = TOK.white, smile = TOK.awsOrange, w = 110 }) => (
...
/* ─────────── Shared lockup body — Bay Area pattern ─────────── */
function Lockup({ city = "ACCRA", showStar = false }) {
  return (
    <div style={{
      position: "relative",
      width: "100%",
      ...
      <div style={{ position: "relative", zIndex: 2 }}>
        ...
        <div style={{
          ...
          <div style={{ height: 1.5, width: 90, background: TOK.ruleLight }} />
          <div style={{ position: "relative", display: "inline-flex", alignItems: "center", justifyContent: "center" }}>
            {/* Star backdrop hugs the city word only */}
            {showStar && (
...
/* ─────────── A · No backdrop (matches Bay Area exactly, with ACCRA) ─────────── */
function LogoA() {
  return (
    <div style={{
      background: TOK.midnight,
      width: "100%", height: "100%",
      display: "flex", alignItems: "center", justifyContent: "center",
      padding: "60px 0",
    }}>
      <Lockup showStar={false} />
    </div>
  );
}

/* ─────────── B · With Ghana black-star backdrop ─────────── */
function LogoB() {
  return (
    <div style={{
      background: TOK.midnight,
      ...
      <Lockup showStar={true} />
    </div>
  );
}

/* ─────────── C · "ACCRA · GHANA" instead of just ACCRA, with subtle star ─────────── */
function LogoC() {
  return (
    <div style={{
      background: TOK.midnight,
      ...
      <Lockup city="ACCRA · GHANA" showStar={true} />
    </div>
  );
}

window.LogoOptions = { LogoA, LogoB, LogoC };
