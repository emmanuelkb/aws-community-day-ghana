...
const KeynoteRoleCard = ({ title, who }) => (
  <div style={{
    ...
    <div style={{ display: "flex", gap: 20, alignItems: "center", marginTop: 8 }}>
      <div style={{
        width: 72, height: 72, borderRadius: 12, background: "rgba(255,255,255,0.08)",
        border: "1px dashed rgba(255,255,255,0.3)", display: "flex", alignItems: "center",
        justifyContent: "center", flexShrink: 0, fontSize: 24, color: "rgba(255,255,255,0.5)",
      }}>?</div>
      <div>
        <div style={{ fontFamily: "'Inter', sans-serif", fontSize: 22, fontWeight: 800, textTransform: "uppercase", letterSpacing: "-0.01em", lineHeight: 1.05 }}>To be announced</div>
        <div style={{ fontSize: 14, color: "rgba(255,255,255,0.7)", marginTop: 8 }}>{title}</div>
      </div>
    </div>
    <p style={{ fontSize: 14, lineHeight: 1.55, color: "rgba(255,255,255,0.6)", margin: "auto 0 0" }}>{who}</p>
  </div>
);

/* ── Panel card ── */
const PanelCard = ({ title, moderator, seats }) => (
  <div style={{
    background: TOKENS.paper, border: `1px solid rgba(26,20,16,0.12)`, borderRadius: 16, padding: "36px",
    display: "flex", flexDirection: "column", gap: 20,
  }}>
    <div>
      <div style={{ fontSize: 11, letterSpacing: "0.22em", textTransform: "uppercase", color: TOKENS.indigo, fontWeight: 700, marginBottom: 10 }}>Panel</div>
      <div style={{ fontFamily: TOKENS.fontDisplay, fontSize: 20, fontWeight: 800, textTransform: "uppercase", letterSpacing: "-0.01em", color: TOKENS.ink, lineHeight: 1.1 }}>{title}</div>
    </div>
    <div>
      <div style={{ fontSize: 11, letterSpacing: "0.16em", textTransform: "uppercase", color: TOKENS.ink2, fontWeight: 700, marginBottom: 8 }}>Moderator</div>
      <div style={{
        display: "inline-flex", alignItems: "center", gap: 8,
        background: TOKENS.indigo, color: "#fff", borderRadius: 6, padding: "7px 14px",
        fontSize: 13, fontWeight: 600,
      }}>{moderator}</div>
    </div>
    <div>
      <div style={{ fontSize: 11, letterSpacing: "0.16em", textTransform: "uppercase", color: TOKENS.ink2, fontWeight: 700, marginBottom: 10 }}>Panel seats</div>
      <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
        {seats.map((seat, i) => (
          <div key={i} style={{ display: "flex", alignItems: "center", gap: 10 }}>
            <div style={{
              width: 6, height: 6, borderRadius: "50%", background: TOKENS.terracotta, flexShrink: 0,
            }} />
            <div style={{ fontSize: 14, color: TOKENS.ink2, lineHeight: 1.4 }}>{seat}</div>
          </div>
        ))}
      </div>
    </div>
  ...
  <div>
      ...
      <Reveal>
        <div style={{ maxWidth: 1100, margin: "0 auto" }}>
          <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 8 }}>
            ...
            <SectionEyebrow>Keynotes</SectionEyebrow>
          </div>
          <SectionHeadline size="large">Opening keynotes</SectionHeadline>
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 16, marginTop: 48 }} className="keynote-grid">
            {KEYNOTE_ROLES.map((k, i) => (
              <KeynoteRoleCard key={i} title={k.title} who={k.who} />
            ))}
          </div>
        </div>
      </Reveal>
      ...
      <Reveal>
        <div style={{ maxWidth: 1100, margin: "0 auto" }}>
          <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 8 }}>
            ...
            <SectionEyebrow>Panels</SectionEyebrow>
          </div>
          <SectionHeadline size="large">Panel discussions</SectionHeadline>
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 16, marginTop: 48 }} className="panels-grid">
            {PANELS.map((p, i) => (
              <PanelCard key={i} title={p.title} moderator={p.moderator} seats={p.seats} />
            ))}
          </div>
        </div>
      </Reveal>
    ...
    <section style={{ background: TOKENS.paper, padding: "64px 56px 120px" }}>
      <Reveal>
        <div style={{ maxWidth: 1100, margin: "0 auto" }}>
          <div style={{
            background: TOKENS.cream, border: `1px solid rgba(26,20,16,0.12)`, borderRadius: 16,
            padding: "48px 52px", display: "flex", justifyContent: "space-between", alignItems: "center",
            flexWrap: "wrap", gap: 32,
          }}>
            <div>
              <SectionEyebrow>Call for speakers</SectionEyebrow>
              <div style={{ fontFamily: TOKENS.fontDisplay, fontSize: "clamp(24px, 3vw, 40px)", fontWeight: 800, textTransform: "uppercase", letterSpacing: "-0.015em", color: TOKENS.ink, lineHeight: 1.05, marginTop: 12, maxWidth: 520 }}>
...
Object.assign(window, { SpeakersPage });
